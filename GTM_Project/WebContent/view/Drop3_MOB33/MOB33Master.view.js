sap.ui.jsview("com.cg.gtm.view.Drop3_MOB33.MOB33Master", {

	/** Specifies the Controller belonging to this View. 
	 * In the case that it is not implemented, or that "null" is returned, this View does not have a Controller.
	 * @memberOf drop3mockups.MOB33.MOB33Master
	 */
	getControllerName: function () {
		return "com.cg.gtm.view.Drop3_MOB33.MOB33Master";
	},

	/** Is initially called once after the Controller has been instantiated. It is the place where the UI is constructed. 
	 * Since the Controller is given to this method, its event handlers can be attached right away.
	 * @memberOf drop3mockups.MOB33.MOB33Master
	 */
	createContent: function (oController) {
		return new sap.m.Page({
			title: "{i18n>MOB33MasterTitle}",
			showNavButton: true,
			navButtonPress: oController.handleNavButtonPress,
			headerContent: new sap.m.Button({
				icon: "sap-icon://sys-help",
				press: oController.handleHelpButtonPress
			}),
			subHeader: new sap.m.Bar({
				contentMiddle: new sap.m.SegmentedButton({
					id: "MOB33SegmentedButton",
					width: "100%",
					buttons: [
						new sap.m.Button({
							text: "{i18n>MOB33TrainButton}",
							press: oController.handleTrainButtonPress
						}),
						new sap.m.Button({
							text: "Depot/Site",
							press: oController.handleDepotButtonPress
						})
					]
				})
			}),
			content: [
				new sap.ui.layout.VerticalLayout({
					id: "MOB33TrainForm",
					width: "100%",
					content: [
						new sap.ui.layout.form.SimpleForm({
							layout: "ResponsiveGridLayout",
							editable: true,
							emptySpanL: 6,
							emptySpanM: 6,
							breakpointM: 1000,
							content: [
								new sap.m.Label({
									text: "{i18n>MOB33FleetLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id: "MOB33FleetInput",
									name: "Fleet",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpFleet, oController]
								}),
								new sap.m.Label({
									text: "{i18n>MOB33TrainLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id: "MOB33TrainInput",
									name: "Train",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpTrain, oController]
								}),
								new sap.m.Label({
									text: "{i18n>MOB33CarLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id: "MOB33CarInput",
									name: "Car",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpCar, oController]
								}),
								new sap.m.Label({
									text: "{i18n>MOB33ZoneLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id: "MOB33ZoneInput",
									name: "Zone",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpZone, oController]
								}),
								new sap.m.Label({
									text: "{i18n>MOB33PrimarySystemLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id: "MOB33PrimarySystemInput",
									name: "Primary System",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpPrimary, oController]
								}),
								new sap.m.Label({
									text: "{i18n>MOB33EquipmentLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id : "MOB33EQT",
									showValueHelp: false,
									valueHelpRequest: [oController.handleValueHelpEq, oController],
									layoutData: new sap.ui.layout.GridData({
										span: "L7 M7 S7"
									})
								}),
								new sap.m.Button({
									text: "Scan",
									icon: "sap-icon://bar-code",
									type: "Reject",
									layoutData: new sap.ui.layout.GridData({
										span: "L5 M5 S5"
									}),
									press: function () {
								    	varScan = "MOB33EQT";
								    	sap.ui.getCore().byId("idMob24MaterialSearchInput").getController().scanNow();
								      },
								})
							]
						})
					]
				}),
				new sap.ui.layout.VerticalLayout({
					id: "MOB33DepotForm",
					width: "100%",
					visible: false,
					content: [
						new sap.ui.layout.form.SimpleForm({
							layout: "ResponsiveGridLayout",
							editable: true,
							emptySpanL: 6,
							emptySpanM: 6,
							breakpointM: 1000,
							content: [
								new sap.m.Label({
									text: "Depot/Site",
									design: "Bold"
								}),
								new sap.m.Input({
									id: "MOB33DepotInput",
									name: "Depot",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpDepot, oController]
								}),
								new sap.m.Label({
									text: "Area",
									design: "Bold",
								}),
								new sap.m.Input({
									id: "MOB33DepotAreaInput",
									name: "Depot Area",
									showValueHelp: true,
									valueHelpRequest: [oController.handleValueHelpDepotArea, oController]
								}),
								new sap.m.Label({
									text: "{i18n>MOB33EquipmentLabel}",
									design: "Bold"
								}),
								new sap.m.Input({
									id : "MOB33EQD",
									showValueHelp: false,
									valueHelpRequest: [oController.handleValueHelpEqD, oController],
									layoutData: new sap.ui.layout.GridData({
										span: "L7 M7 S7"
									})
								}),
								new sap.m.Button({
									text: "Scan",
									icon: "sap-icon://bar-code",
									type: "Reject",
									layoutData: new sap.ui.layout.GridData({
										span: "L5 M5 S5"
									}),
									press: function () {
								    	varScan = "MOB33EQD";
								    	sap.ui.getCore().byId("idMob24MaterialSearchInput").getController().scanNow();
								      },
								})
							]
						})
					]
				})
			],
			footer: new sap.m.Toolbar({
				content: [
					new sap.m.ToolbarSpacer(),
					new sap.m.Button({
						text: "{i18n>MOB33SearchButton}",
						icon: "sap-icon://search",
						type: "Emphasized",
						press: [oController.handleSearchButtonPress, oController]
					})
				]
			})
		});
	}

});