sap.ui.controller("com.cg.gtm.view.Mob18CostMsater", {
...
var inputPlant = sap.ui.getCore().byId("inputPlant_Cost");
...
//	}
	check : function(){
		var valError = 0;
		if(  null == sap.ui.getCore().byId("inputCostno").getValue().trim() || sap.ui.getCore().byId("inputCostno").getValue().trim() == "")
  	  {
			sap.ui.getCore().byId("inputCostno").setValueState(sap.ui.core.ValueState.Error);
			   valError = 1;
  	  }
...
function callProjSearchHelp_Cost(){
			 ...
			 var aData = jQuery.ajax({   
		         ...
		         success : function(data, textStatus, jqXHR) {
	       	 	...
	       	 	for(var i=0;i<jsonObj.length;i++) {
	       	 		var WBSElementNo = jsonObj[i].WBSElementNo;
	       	 		var WBSElementDesc = jsonObj[i].WBSElementDesc;
	       	 		var objWBS = {"key": WBSElementNo, "detail": WBSElementDesc};
	       	 		arrWBS.push(objWBS);
	       	 	}*/
	...
	var selectCustProj = sap.ui.getCore().byId("inputcustomer_cost");
	selectCustProj.setVisible(false);
	/*Setting Value for Proj/Cust - End */	
}
function callCustomerSearchHelp_Cost(){
	//openSplashScreen();//splash screen
	var jsonCustProj = null;
	var demo = sap.ui.getCore().byId("demoswitch");  
		...
		if(url1 == "Fail")
		 ...
		 var aData = jQuery.ajax({   
	       ...
	       /* 	var arrCust = [];
       	 	...
       	 	for(var i=0;i<jsonObj.length;i++) {
       	 		var custNo_cost = jsonObj[i].CustomerNo;
       	 		var custName_cost = jsonObj[i].CustomerName;
       	 		var objCust = {"key": CustomerNo, "detail": CustomerName};
...
function callStorageLocation_cost(jsonStore) {
	...
	if(url1 == "Fail")
	 ...
	 var aData = jQuery.ajax({   
         ...
         success : function(data, textStatus, jqXHR) {
        	...
        	var oJSONFromStorage = new sap.ui.model.json.JSONModel(jsonFromStore);
    		var fromStorage = sap.ui.getCore().byId("MvtType_cost");
    		fromStorage.setModel(oJSONFromStorage);
...
function callReasoncode_cost(key){
	var jsonStore = null;
	var demo = sap.ui.getCore().byId("demoswitch"); 
	if (demo.getState() == true) {
		jsonStore = mob18Res();
	...
	if(url1 == "Fail")
	 ...
	 var aData = jQuery.ajax({   
         ...
         success : function(data, textStatus, jqXHR) {
        	...
        	var oJSONFromStorage = new sap.ui.model.json.JSONModel(jsonFromStore);
    		var fromStorage = sap.ui.getCore().byId("ResType_cost");
    		fromStorage.setModel(oJSONFromStorage);
